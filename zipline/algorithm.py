...
# perf_tracker
# perf_tracker.handle_simulation_end
...
class TradingAlgorithm(object):
    ...
    *args, **kwargs
	...
	#### futures_metadata:(期货数据)，数据类型和股票数据一致，区别在于用于期货信息
    identifiers : list, optional
        Any asset identifiers that are not provided in the
        equities_metadata, but will be traded by this TradingAlgorithm.
	####身份识别：list ,任何在股票市场没有提供身份识别符的，通过tradingAlgorithm 进行交易
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.long=True
        # 在进行handle_data之前，获取可能需要的历史行情数据，并存入缓存
        
        self.rolling_count =kwargs.pop('rolling_count',[1,'1d',['close']])
        ...
        # set the capital base，没有提供资金就是用默认资金10000
        self.capital_base = kwargs.pop('capital_base', DEFAULT_CAPITAL_BASE)
        self.sim_params = kwargs.pop('sim_params', None)
		#如果没有提供，sim_params，则使用下面的sim_params进行替换，是一个类有很多方法何函数
        ...
        self.event_manager.add_event(
            zipline.utils.events.Event(
                ...
                # 我们传递handle_data .__ func__来获取未绑定的方法。
                # 我们将显式传递算法再次绑定它。
    ...
    def handle_data(self, data):
        ...
        # 与交易控制不同，除非下订单，否则账户控制可以改变每个柱。 
        # 因此，无论算法是否下订单，都必须检查每个条
...
""".strip().format(class_name=self.__class__.__name__,
    ...
    def _create_data_generator(self, source_filter, sim_params=None):
        ...
        skipped.
        # 是一个按日期排序顺序接收事件的方法，
        #对于那些应该由zipline处理的事件返回True，对于那些应该跳过的事件返回False。
        """
        ...
        date_sorted = date_sorted_sources(*self.sources) #进行迭代的数据，调用一次获取一次数据)
    ...
    def _create_generator(self, sim_params, source_filter=None):
        ...
        ::source_filter 数据源过滤:: is a method that receives events in date
        sorted order, and returns True for those events that should be
        processed by the zipline, and False for those that should be
        skipped.
        """
        # 是一个按日期排序顺序接收事件的方法，对于那些应该由zipline处理的事件返回True
		#，对于那些应该跳过的事件返回False。
    ...
    def run(self, sources, overwrite_sim_params=True,
        ...
        if overwrite_sim_params:
            if hasattr(source, 'start'):#判断source，里面是否有相应的属性以及方法
                self.sim_params.period_start = sources[1] # source.start
            if hasattr(source, 'end'):
                self.sim_params.period_end =sources[2] # source.end #
        ...
        # Create history containers self.rolling_count
        #print self.rolling_count,u'rolling_count数据'
        #print sources[1]
        #raw_input()
        for field_name in self.rolling_count[2]:
            spec_key = HistorySpec.spec_key(self.rolling_count[0],self.rolling_count[1],field_name,True )
            self.history_specs[spec_key] = HistorySpec(self.rolling_count[0],
        ...
        # perf dictionary
		# 通过simulated_trading 循环遍历，每次迭代返回一个表现字典5
        perfs = []
        ...
        daily_stats.to_csv('D:\\nihao2.csv',encoding = 'gbk')
    ...
    def order(self, sid, amount,
        ...
        #股票数目检测，对于分为买入和卖出，当卖出这里暂时设定不能卖空
        # 卖出，查看股票账户中的股票数量，并选择卖出数量
        ...
        elif self.portfolio.cash >= last_price*amount and amount > 0:
        ...
        #print self.portfolio.cash,sid,amount,u'数据量'
        return self.blotter.order(sid, amount, style,self.commission,give_price = give_price)
    ...
    def validate_order_params(self,
        ...
        #用于验证订单API函数的参数的辅助方法。 如果找到无效参数，
        #则引发UnsupportedOrderParameters
        """
    ...
    def get_history_spec(self, bar_count, frequency, field, ffill):
        ...
        if spec_key not in self.history_specs:
            ...
            # 给history_container指定空间，确保能够进行改变
            
            self.history_container.ensure_spec(
                spec, self.datetime, self._most_recent_data,
            )
            # 这里我们需要对history_container 向前追溯bar_count个交易日，并将其储存在history_container里面
        return self.history_specs[spec_key]

    @api_method
    @require_initialized(HistoryInInitialize())
    def history(self, bar_count, frequency, field, ffill=True):
        # 下面代码为对rollingpanel 进行初始化，仅仅在history,进行的时候进行执行，并在第一次执行的时候进行初始化
