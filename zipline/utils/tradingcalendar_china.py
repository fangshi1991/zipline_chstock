...
# 由于国内的交易日期和国外并不一致，所以这里讲国外的数据调整为国内的交易日期数据
'''
import pandas as pd
import pytz
...
def get_non_trading_days(start, end):
    ...
    tradeing_date = [canonicalize_datetime(i)for i in (get_benchmark_returns('000001',start,end)).index]
    ...
    #bias_day = [p for p in all_date if (p not in tradeing_date and p not in non_trading_days)]
    non_trading_days = non_trading_days+[p for p in all_date if (p not in tradeing_date and p not in non_trading_days)]
...
#print non_trading_days[3700:3800]
...
def get_open_and_close(day, early_closes):
    ...
    market_open = pd.Timestamp(
        datetime(
            ...
            day=day.day,
            hour=9,
            minute=25)).tz_localize('UTC')
        #tz='US/Eastern').tz_convert('UTC')
    # 1 PM if early close, 3 PM otherwise
    close_hour = 15 if day in early_closes else 15
    market_close = pd.Timestamp(
        datetime(
            year=day.year,
            month=day.month,
            day=day.day,
            hour=close_hour)).tz_localize('UTC')
        #tz='US/Eastern').tz_convert('UTC')
