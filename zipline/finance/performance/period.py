...
class PerformancePeriod(object):
    ...
    def __init__(
        ...
        #print u'performancePeriod里面的时间问题'
        #print 'open',period_open
        #print 'close',period_close
        #raw_input()
        ...
        # 以资产为基准，以前支付价格差异的头寸的最后一次销售价格，
        # 例如 期货。 此dt不是计算完成的前一分钟，
        # 而是在期间开始之前或执行时的价格的最后一个销售价格。
    ...
    def calculate_performance(self):
        ...
        #total_at_start 为开始是市值与现金之和
        total_at_start = self.starting_cash + self.starting_value
        #ending_cash 为结束时候的现金，为开始的现金加上，期间现金的流动，加上分红
        # self.period_cash_flow 由每次handle 执行进行处理，由handle_execution 与_calculate_execution_cash_flow 共同获得
    ...
    def handle_execution(self, txn):
        #进行执行
        #print self.period_cash_flow,u'period_cash_flow 现金情况'
        self.period_cash_flow += self._calculate_execution_cash_flow(txn)
        #print self.period_cash_flow,u'period_cash_flow 现金情况2222222'
        ...
        if self.keep_transactions:
            #print u'在这里的情况，today_handle_execution'
            try:
                
                self.processed_transactions[txn.dt].append(txn)
                #print self.processed_transactions
            except KeyError:
                self.processed_transactions[txn.dt] = [txn]
                #print self.processed_transactions,u'processed的情况'
