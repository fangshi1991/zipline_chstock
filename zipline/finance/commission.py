...
class OrderCost(object):
    ...
    def calculate(self, transaction):
        ...
        returns a tuple of:
        (per share commission, total transaction commission)
        """# 本部分计算交易手续费，返回每股交易费用，总费用
        if transaction.amount == 0:
            return 0.0,0.0
        if transaction.amount > 0:
        #最低费用为5元
            if transaction.price*(self.open_commission)*transaction.amount<self.min_commission:
                return self.min_commission*1.0000/transaction.amount,self.min_commission
            else:
                return transaction.price*(self.open_commission), transaction.price*(self.open_commission)*transaction.amount
        else:
            if transaction.price*(self.close_commission)*abs(transaction.amount)<self.min_commission:
                return self.min_commission*1.0000/transaction.amount+transaction.price*self.close_tax,self.min_commission+transaction.price*(self.close_tax)*abs(transaction.amount)
            else:
